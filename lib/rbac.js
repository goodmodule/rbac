...
RBAC.prototype.createPermissions = function(permissions, cb) {
	var tasks = {};
	permissions = RBAC.permissionsFromObject(permissions);
 ...
 * @param  {Object}  grants      [description]
...
RBAC.prototype.canAll = function(role, permissions, cb) {
	...
	this._traverseGrants(role, function(err, item) {
		...
		if(item instanceof Permission && _.indexOf(permissionNames, item.getName()) !== -1 && !founded[item.getName()]) {
