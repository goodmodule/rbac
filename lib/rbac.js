...
RBAC.prototype.register = function(item) {
	...
	if(item.getRBAC() !== this) {
		throw new Error('RBAC is different');
	}
	...
	if(item instanceof Permission) {
		this._permissions[name] = item;
	}else if(item instanceof Role) {
		this._roles[name] = item;
	}
 ...
 */
RBAC.prototype.remove = function(item) {
	if(item.getRBAC() !== this) {
		throw new Error('RBAC is different');
	}
	...
	if(!this._items[name]) {
		throw new Error('Item is unregistered');
	}

	//revoke from hierarchy
	for(var name in this._roles) {
 ...
 * @return {Boolean}          
 */
RBAC.prototype.can = function(roleName, action, resource) {
	var item = this.getRole(roleName);
	return item ? item.can(action, resource) : false;
};
 ...
 * @return {[type]}            
 */
RBAC.prototype.canAny = function(roleName, permissions) {
	var item = this.getRole(roleName);
	return item ? item.canAny(permissions) : false;
};
 ...
 * @return {[type]}            
 */
RBAC.prototype.canAll = function(roleName, permissions) {
	var item = this.getRole(roleName);
	return item ? item.canAll(permissions) : false;
};
...
RBAC.prototype.getRoleNames = function() {
	var names = [];
