...
MongooseStorage.prototype.remove = function(item, cb) {
	...
	//remove from grants 
	this.model.update({ grants: name }, { $pull: { grants: name } }, { multi: true }, function(err, num) {
		if(err) {
			return cb(err);
		}

		//remove item
		_this.model.remove({ name: name }, function(err, num) {
			if(err) {
				return cb(err);
			}
...
MongooseStorage.prototype.grant = function(role, child, cb) {
	...
	this.model.update({ name: name, type: type.ROLE }, { $addToSet: { grants: childName } }, function(err, num) {
		if(err) {
			return cb(err);
		}
